...
// Completely redesigned custom state dropdown to fix layout breaking on mobile
  ...
  return (
    <div className="relative w-full" ref={dropdownRef}>
      <button
        ...
        className="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm text-left focus:outline-none focus:border-[#C1272D] focus:ring-1 focus:ring-red-100 bg-white transition flex items-center justify-between"
      >
      ...
      {isOpen && (
        <div className="absolute z-50 left-0 right-0 mt-1.5 bg-white border border-gray-200 rounded-xl shadow-xl max-h-60 overflow-y-auto">
          {placeholder && (
            <div
              onClick={() => { onChange(""); setIsOpen(false); }}
              className="px-4 py-2.5 text-sm text-gray-400 hover:bg-gray-50 cursor-pointer italic transition-colors"
            >
          ...
          {options.map((o) => (
            <div
              ...
              className={`px-4 py-2.5 text-sm text-gray-700 hover:bg-red-50 hover:text-[#C1272D] cursor-pointer transition-colors ${
                value === o ? "bg-red-50/70 text-[#C1272D] font-bold" : ""
              }`}
            >
              {o}
            </div>
...
function AddJobModal({ onClose, onCreateJob }) {
  ...
  const [form, setForm] = useState({
    ...
    language: "", requiredSkills: ["Communication", "Teamwork"],
    ...
    switch (step) {
      case 0: return (
        <div className="space-y-4 md:space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-5">
            <div><Label text="Your Company Name" required /><Input placeholder="Your company name" value={form.companyName} onChange={v => set("companyName", v)} /></div>
            <div><Label text="Job title" required /><Input placeholder="Ex. Sales manager" value={form.jobTitle} onChange={v => set("jobTitle", v)} /></div>
          </div>
          <div>
          ...
          <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 md:gap-4">
              <Input placeholder="Min. Salary" value={form.minSalary} onChange={v => set("minSalary", v)} />
              <Input placeholder="Max. Salary" value={form.maxSalary} onChange={v => set("maxSalary", v)} />
            </div>
          </div>
          <div>
      ...
      case 1: return (
        <div className="space-y-4 md:space-y-6">
          ...
          <div><Label text="Language Preference" required /><Select placeholder="select" value={form.language} onChange={v => set("language", v)} options={["English", "Hindi", "Bengali", "Tamil", "Telugu", "Kannada", "Marathi"]} /></div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-5">
            <div>
      ...
      case 2: return (
        <div className="space-y-4 md:space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-5">
            <div><Label text="Number of Open Positions" required /><Input placeholder="Enter number of positions" value={form.openPositions} onChange={v => set("openPositions", v)} /></div>
            <div>
              <Label text="Salary Range (per annum)" required />
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <Input placeholder="Min. Salary" value={form.scrMinSalary} onChange={v => set("scrMinSalary", v)} />
                <Input placeholder="Max. Salary" value={form.scrMaxSalary} onChange={v => set("scrMaxSalary", v)} />
              </div>
            </div>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-5">
            ...
            <div><Label text="Job Location" required /><Input placeholder="Enter job location" value={form.jobLocation} onChange={v => set("jobLocation", v)} icon="📍" /></div>
          </div>
          <div>
      ...
      case 3: return (
        <div className="space-y-4 md:space-y-6">
          ...
          <div>
            ...
            <div className="border border-gray-200 rounded-xl px-4 py-4 bg-white space-y-2 min-h-[120px]">
              {form.responsibilities.map((r, i) => (
                <div key={i} className="flex items-start gap-2 text-sm text-gray-700">
                  <span className="text-[#C1272D] mt-0.5">•</span>
                  <span className="flex-1">{r}</span>
                  <button onClick={() => set("responsibilities", form.responsibilities.filter((_, j) => j !== i))} className="text-gray-300 hover:text-red-400 text-base">×</button>
                </div>
              ))}
              <button onClick={() => set("responsibilities", [...form.responsibilities, "New responsibility"])}
                className="text-[#C1272D] text-xs font-semibold hover:underline mt-1">+ Add more</button>
            </div>
          </div>
        </div>
      );
      ...
      case 6: return (
        <div className="space-y-4 md:space-y-6">
          ...
          <div>
            ...
            <div className="border-2 border-dashed border-red-200 rounded-2xl p-6 md:p-10 flex flex-col items-center justify-center gap-2 bg-red-50/30 cursor-pointer hover:bg-red-50/50 transition">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-5">
            <div><Label text="Company Name" required /><Input placeholder="Enter your company name" value={form.companyNameDetail} onChange={v => set("companyNameDetail", v)} /></div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-5">
            <div><Label text="Phone Number" required /><Input placeholder="+1 (555) 123-4567" value={form.phone} onChange={v => set("phone", v)} icon="📞" /></div>
            <div><Label text="Website URL" /><Input placeholder="https://www.company.com" value={form.website} onChange={v => set("website", v)} /></div>
          </div>
          <div><Label text="Current Location" required /><Input placeholder="City, State" value={form.location} onChange={v => set("location", v)} icon="📍" /></div>
        </div>
      );
      case 7: return (
        <div className="space-y-4 md:space-y-6">
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end md:items-center justify-center p-0 md:p-4 animate-fade-in" style={{ background: "rgba(0,0,0,0.45)", backdropFilter: "blur(4px)" }}>
      ...
      <div className="bg-white rounded-t-3xl md:rounded-3xl w-full max-w-7xl shadow-2xl flex flex-col md:flex-row overflow-hidden max-h-[92vh] md:max-h-[90vh]">
        ...
        {/* ── Left/Top Sidebar (Changes layout on mobile/tablet vs desktop) ── */}
        <div className="w-full md:w-72 flex-shrink-0 bg-gray-50 border-b md:border-b-0 md:border-r border-gray-100 flex flex-col py-4 md:py-8 px-4 md:px-5">
          ...
          {/* Scrollable layout for step tabs on horizontal mobile environments */}
            ...
            {jobs.map((job, index) => (
              <div key={job.id}
                ...
                <div>
                  ...
                  {/* ── Fixed Metadata Rows for Mobile Responsiveness ── */}
                ...
                {/* ── Action Footer Row Wrap Protection ── */}
