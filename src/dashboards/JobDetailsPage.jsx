...
// ── Stage color mapping ──
function getStageStyle(stageName) {
  const name = (stageName || '').toLowerCase();
  if (name.includes('new') || name.includes('applied')) return { bg: '#EFF6FF', text: '#1D4ED8', dot: '#3B82F6' };
  if (name.includes('screen') || name.includes('review')) return { bg: '#FFFBEB', text: '#B45309', dot: '#F59E0B' };
  if (name.includes('interview')) return { bg: '#F5F3FF', text: '#6D28D9', dot: '#8B5CF6' };
  if (name.includes('offer') || name.includes('select')) return { bg: '#ECFDF5', text: '#065F46', dot: '#10B981' };
  if (name.includes('reject') || name.includes('decline')) return { bg: '#FFF1F2', text: '#9F1239', dot: '#F43F5E' };
  return { bg: '#F9FAFB', text: '#374151', dot: '#6B7280' };
}
...
function JobDetailsPage() {
  ...
  // ✅ Note: applied will also be set to true if stageData is fetched from API
  ...
  const { getPublishedJobs, applyJob, applyLoading, jobs, jobsLoading, getJobStage } = useResume();
  ...
  // ── NEW: Fetch job stage when job loads ──
  useEffect(() => {
    if (!job) return;
    ...
    const fetchStage = async () => {
      ...
      const result = await getJobStage(Number(candidateId), Number(id));
      if (result) {
        setStageData(result);
        setApplied(true); // ✅ Stage aa gayi matlab apply ho chuka hai
      }
  ...
  const stageStyle = getStageStyle(stageData?.stage_name);
      ...
      <div className="px-4 sm:px-6 lg:px-10 py-5">
        ...
        <div className="flex gap-5 items-start">
          ...
          <aside className="hidden lg:block w-80 flex-shrink-0 sticky top-20">
            ...
            {/* ── NEW: Application Stage Card ── */}
            {(stageLoading || stageData) && (
              <div className="bg-white rounded-2xl p-5 border border-gray-100 shadow-xs hover:shadow-md transition-shadow duration-300 mb-4">
                <h2 className="text-base font-extrabold text-[#C8102E] mb-3.5 border-b border-gray-50 pb-2 flex items-center gap-2">
                ...
                {stageLoading ? (
                  <div className="flex items-center gap-2.5 py-3">
                    <div className="w-4 h-4 border-2 border-[#C8102E] border-t-transparent rounded-full animate-spin flex-shrink-0" />
                    <span className="text-[13px] text-gray-400 font-medium">Fetching your stage...</span>
                  </div>
                ) : stageData ? (
                  <div className="flex flex-col gap-3">
                    ...
                    <div
                      ...
                      style={{ background: stageStyle.bg, borderColor: stageStyle.bg }}
                    >
                      <span
                      ...
                      />
                      <div>
                        <p className="text-[11px] font-semibold uppercase tracking-wider" style={{ color: stageStyle.dot }}>Current Stage</p>
                        <p className="text-[15px] font-extrabold leading-tight" style={{ color: stageStyle.text }}>
