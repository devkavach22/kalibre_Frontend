                            ...
                            >
                                <div className="flex items-center justify-between gap-3">
                                    <div className="flex items-center gap-3 sm:gap-4 min-w-0 flex-1">
                                        <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-xl bg-gradient-to-br from-slate-50 to-slate-100 border border-slate-200/60 flex items-center justify-center font-bold text-slate-700 tracking-wider group-hover:from-red-50 group-hover:to-rose-100 group-hover:text-red-600 transition-all duration-300 shrink-0 text-sm">
                                            {company.company_name ? company.company_name.substring(0, 2).toUpperCase() : 'CO'}
                                        </div>
                                        <div className="space-y-1 min-w-0 flex-1">
                                            <h3 className="text-sm sm:text-base font-bold text-slate-800 tracking-tight leading-tight group-hover:text-red-600 transition-colors break-words">
                                                {company.company_name}
                                            </h3>
                                            <div className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-3 text-xs sm:text-sm text-slate-600 font-semibold pt-0.5">
                                                ...
                                                <span className="flex items-center gap-1.5 text-slate-500 font-medium">
                                                    ...
                                                    <span className="break-words">{company.location || "No location listed"}</span>
                                                </span>
                                            </div>
                                        </div>
                                    </div>

                                    <div className="hidden sm:block text-slate-300 group-hover:text-red-400 transform group-hover:translate-x-1 transition-all shrink-0">
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="space-y-3">
                                ...
                                {!loading && !error && recruiters.map((hr) => (
                                    <div
                                        key={hr.id}
                                        className="group bg-white rounded-xl p-4 border border-slate-200/80 hover:border-red-200 shadow-sm hover:shadow-[0_8px_20px_rgba(220,38,38,0.04)] transform hover:-translate-y-0.5 transition-all duration-300 flex items-center gap-3 sm:gap-4"
                                    >
                                        <div className="w-10 h-10 sm:w-11 sm:h-11 rounded-full bg-gradient-to-br from-slate-50 to-slate-100 border border-slate-200/60 flex items-center justify-center font-bold text-xs text-slate-600 group-hover:from-red-50 group-hover:to-rose-100 group-hover:text-red-600 transition-all duration-300 shrink-0">
                                            {hr.name ? hr.name.split(' ').filter(Boolean).map(n => n[0]).join('') : 'HR'}
                                        </div>

                                        <div className="space-y-1 min-w-0 flex-1">
                                            <h4 className="text-sm font-bold text-slate-800 leading-tight group-hover:text-red-600 transition-colors break-words">
                                                {hr.name}
                                            </h4>
